...
var MAC={
    ...
    'Shorten': {
        ...
        'Get':function(url,call){
            ...
            MAC.Ajax('//api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long='+ encodeURIComponent(url),'get','jsonp','',function(r){
                if (r.code == 1) {
                    if($('.mac_shorten').length>0) {
    ...
    'sort_btn':function($sort_btn,$sort_list,$sort_li){
        $($sort_btn).click(function(index){$($sort_list).each(function(index, element) {var sort_ul = $(this).find($sort_li).toArray().reverse();$(this).html('<div class="sort_li"></div>');$(this).find("div").replaceWith(sort_ul);});});
    }
}
...
$(function(){
    ...
    MAC.Timming();
    //排序初始化
    MAC.sort_btn('.sort_btn','.content_playlist','li');
